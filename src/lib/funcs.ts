...
export const returnPromptArray = (stages: stage[], all: boolean = false) => {
	let promptArray: string[] = [];
	//Add a filter Param, so that people choosing just prompts can use
	stages.map((stage: stage) => {
		if (stage.images.some((image) => image.chosen) || all) {
